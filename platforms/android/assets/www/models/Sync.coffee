...
#                      error: (error) => @log "Could not create log file #{JSON.stringify(error)}"
  ...
  replicateFromServer: (options, completeMsgId) ->
    options = {} if !options
    filter = (doc) ->
...
#    Coconut.menuView.checkReplicationStatus();
  ...
  replicateForms: (options, completeMsgId) ->
    ...
    opts =
        ...
        complete: (result) ->
            if typeof result != 'undefined' && result.ok
              ...
              if typeof completeMsgId != 'undefined' && completeMsgId != null
                $(completeMsgId).append "replicateForms complete; potential problem:" + jsonResult + '<br/>'
        error: (result) ->
            Coconut.debug "error: Replication error: " + JSON.stringify result
        timeout: 60000
    _.extend options, opts
  ...
  replicateToServer: (options, completeMsgId) ->
    options = {} if !options
    filter = (doc) ->
      if doc._id != "_design/by_clientId" && doc._id != "_design/by_serviceUuid" && doc._id != "SyncLog" && doc._id != "coconut.config" && doc._id != "coconut.config.local" && doc._id != "version" && doc.noClientPush != "true"
          return doc
...
#        password:account.password
      complete: (result) ->
        if typeof result != 'undefined' && result.ok
          ...
          if typeof completeMsgId != 'undefined' && completeMsgId != null
            $(completeMsgId).append "replicateToServer complete; potential problem:" + jsonResult + '<br/>'
      error: (result) ->
        Coconut.debug "error: Replication error: " + JSON.stringify result
      timeout: 60000
    _.extend options, opts
