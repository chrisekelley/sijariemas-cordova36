...
Backbone.sync.defaults.db.put(by_PatientIdDesignDoc).then(function (doc) {
    // design doc created!
    console.log("by_PatientId created")
    Backbone.sync.defaults.db.query('by_PatientId', {stale: 'update_after'})
}).catch(function (err) {
    if (err.name === 'conflict') {
